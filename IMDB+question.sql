...
select Year, count(title) as number_of_movies from movie group by year order by year;

select month(date_published) as month_num, count(title)
 as number_of_movies from movie group by month(date_published) order by month(date_published) ;
...
select count(*) as num_movies, country from movie where country in ('usa', 'india') group by country;
...
-- Type your code below:
select genre, count(*) as Genre_Count from genre g 
...
from (select movie_id from genre group by movie_id having count(*) = 1) as one_genre_movie;
...
inner join movie m on m.id = g.movie_id group by genre order by avg_duration;
...
select  production_company, count(title) as movie_count, rank() over(order by count(movie_id) desc) as prod_company_rank
...
select genre, count(*) as movie_count from genre g 
join ratings r using (movie_id) 
join movie m on m.id = r.movie_id
where total_votes > 1000 and country = 'USA' and year = 2017 and month(date_published) = 3
 group by genre;
...
select count(*) as Num_movies from movie m join ratings r on m.id = r.movie_id 
